...
int main() {
  ...
  if (netserverinit("localhost",1) == 0) { 
  ...
  int fd = 0;
  char *r_buffer = (char*)malloc(4);
  const char *w_buffer = (const char*)malloc(4);
  size_t nbyte_r = 4;
  size_t nbyte_w = 4;
  ...
  free(r_buffer);
  free((char*)w_buffer);
  r_buffer = (char*)malloc(4);
  w_buffer = (const char*)malloc(4);
