...
int netread(int client){
	...
	int count_nbyte = 0;
	while(count_nbyte < sizeof(size_t)){
		count_nbyte += recv(client, &nbytes, sizeof(size_t), 0);
		if(count_nbyte == 0){
			continue;
		}
	}
...
int main(int argc, char ** argv){
	...
	//init mutex
	if (pthread_mutex_init(&lock, NULL) != 0 || pthread_mutex_init(&queuetex, NULL) != 0 || pthread_mutex_init(&socktex, NULL) != 0 || pthread_mutex_init(&conf, NULL) != 0 || pthread_mutex_init(&conf2, NULL) != 0){
        printf("\n mutex init failed\n");
        return 1;
    }
